-- | Compose.
--
-- Functional programming is a lot about composing smaller pieces to bigger
-- programs. Rhine gives you many ways to compose!
--
-- One of them is this operator: >->
--
-- A composition is written as `sf1 >-> sf2`, where `sf1` and `sf2` are clocked
-- signal functions (`ClSF`s). For two `ClSF`s to be composable, the _output_
-- type of the first one has to match the _input_ type of the second one!
module Koan where
