...
--
-- So far, we always had one clock, and added it to our program right at the end
-- with the @@ operator. But what actually happens when we apply @@?
--
...
--
-- We end up with a complete program that consists of a signal function which
-- produces, processes, and consumes data, and a clock which drives the signal
-- function at a specific rate.
--
-- But unfortunately, some very strange error messages have crept in. This is
-- because we have used the general composition operator >>> instead of the
-- Rhine-specific operator >->, and the type checker is confused which of @@ and
-- >>> to apply first. Can you fix it?
module Koan where
