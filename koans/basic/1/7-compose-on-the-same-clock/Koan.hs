...
--
-- You have seen before how important it is to get the rate of a clock right. What
-- if we tried to compose components that are specified to run at different rates?
-- This cannot work correctly! And therefore, it is forbidden in Rhine. See the
-- type error for yourself! Can you fix the program such that all components run at
-- one message per second again?
module Koan where
