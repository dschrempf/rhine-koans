...
--
-- The last problem got quite verbose, and fiddling around with nested tuples isn't
-- fun. Fortunately, Haskell has a language extension that provides very useful
-- syntax for data flow constructs like signal functions! It is called "arrow
-- notation", and you can read a bit more about it here:
-- https://www.haskell.org/arrows/. Have a look how the code of the previous koan
-- can be cleaned up with it.
module Koan where
...
-- proc is a keyword. Think of it like a lambda expression! But why does GHC
-- spit out a nasty parse error here? Read through the following to find out!
