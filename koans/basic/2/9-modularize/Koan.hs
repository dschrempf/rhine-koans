...
-- | Modularize.
--
-- Let's clean up the code. Ideally, we don't have one big monolithic function, but
-- rather several reusable, independent ones. A typical pattern is to separate
-- computations from side effects, so let us do that here!
module Koan where
